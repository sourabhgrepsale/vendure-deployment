...
IMAGE="storefront"
APP_DIR="/Users/sourabh/vendure/storefront"
...
echo -e "${YELLOW}=== Building and Deploying Storefront to Sandbox Cluster ===${NC}"
echo ""
...
kubectl -n "${NAMESPACE}" set image deployment/storefront app="${FULL}"
echo -e "${GREEN}✓ Deployment updated${NC}"
echo ""

# Wait for rollout
echo -e "${YELLOW}[4/4] Waiting for rollout to complete...${NC}"
kubectl -n "${NAMESPACE}" rollout status deployment/storefront
...
echo ""
echo "Storefront pods:"
kubectl -n "${NAMESPACE}" get pods -l app=storefront -o wide || true
echo ""
echo -e "${GREEN}Storefront is now running in sandbox cluster!${NC}"
echo ""
echo "To view logs:"
echo "  kubectl logs -n ${NAMESPACE} deployment/storefront -f"
echo ""
